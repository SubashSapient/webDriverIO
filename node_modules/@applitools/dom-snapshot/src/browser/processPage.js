...
const makeLogTimestamp = require('./logTimestamp');
const noop = require('./noop');
const makeSessionCache = require('./sessionCache');

function processPage(doc = document, debug) {
  const logTimestamp = debug ? makeLogTimestamp(Date.now()) : noop;
  logTimestamp('processPage start');
  const sessionCache = makeSessionCache({log: logTimestamp});
  ...
  function doProcessPage(doc, pageUrl = doc.location.href) {
    ...
    const resourceUrlsAndBlobsPromise = getResourceUrlsAndBlobs({documents: docRoots, urls}).then(
      result => {
